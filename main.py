        ...
        # Handle 'message' event (user sends text or clicks keyboard button)
            ...
            if message_type == 'text':
                ...
                # NEW: If user is in agent mode and sends a message, forward to agent
                    ...
                    # Store message in agent_conversations for the dashboard to pull
                    if sender_id not in agent_message_queues:
                         agent_message_queues[sender_id] = asyncio.Queue()
                    ...
                    # Put message to the agent's specific queue, and broadcast its availability
                    # For simplicity, we'll put it into a general broadcast queue for dashboard updates
...
async def agent_dashboard_stream(authenticated: bool = Depends(verify_monitor_credentials)):
    ...
    """
    from sse_starlette.sse import EventSourceResponse # Import here or at top

    async def event_generator():
        while True:
            try:
                # Wait for a new message or conversation update
